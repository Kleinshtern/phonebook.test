<script setup>
    ...
    onMounted(() => {
        ...
        store.fetchContact(props.id)
            .then(() => {
                id.value = props.id;
                firstname.value = store.contact.first_name;
                lastname.value = store.contact.last_name ?? "";
                company.value = store.contact.company ?? "";
                phoneType.value = store.contact.phone_type;
                phoneNumber.value = store.contact.phone;
...
<template>
    <div class="flex flex-col gap-2 items-start">
        ...
        <v-form class="flex flex-col gap-2 w-full lg:w-2/3 mx-auto" @submit.prevent="submitForm">
            <h6 class="text-h6 font-weight-bold">Редактирование контакта</h6>
            ...
            <v-btn type="submit" color="primary" text="Сохранить" class="text-white"></v-btn>
        </v-form>
    </div>
</template>
